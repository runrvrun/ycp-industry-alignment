...
**Submission flow:** `SurveyWizard` POSTs `{ reviewer: {name, office}, responses: {...} }` to `app/api/responses/route.ts`, which validates `reviewer.name` and each item id/verdict against `taxonomy-data.ts` before writing a `Submission` + related `Response` rows via the Prisma singleton (`app/lib/prisma.ts`). On failure, the reviewer stays on Screen 3 with an inline error and can retry — there is no local/JSON download fallback (removed; it triggered an unwanted file-download prompt and wasn't something reviewers would use).

**Data model (`prisma/schema.prisma`):** `Submission` (reviewer name/office/timestamp) has many `Response` rows (`itemId`, `verdict`, `comment`) — normalized rather than one JSON blob per submission, so `/results` can tally verdicts per item cheaply.

**Two SQLite backends, chosen at runtime (`app/lib/prisma.ts`):** Prisma 7 requires an explicit driver adapter even for SQLite. Locally, `DATABASE_URL` (`.env`, default `file:./dev.db`) is used with `@prisma/adapter-better-sqlite3` against a plain file — `npx prisma migrate dev` manages this. In production (e.g. Vercel), there's no persistent filesystem for a SQLite file to live on, so the app instead talks to a hosted libSQL database on Turso via `@prisma/adapter-libsql`, selected automatically whenever `TURSO_DATABASE_URL`/`TURSO_AUTH_TOKEN` are set (see `.env.example`) — leave those unset locally so dev keeps using the local file. Because Prisma's migration engine doesn't speak the `libsql://` protocol, schema changes are applied to Turso manually: run `prisma migrate dev` locally to generate the migration SQL under `prisma/migrations/`, then apply that file to Turso with `turso db shell <db-name> < prisma/migrations/<new-migration>/migration.sql`.
